...
>\
>アプリとブラウザの制御 → スマート アプリ コントロールの設定 →「オフ」を選択。  
>※2026年4月累積更新（KB5083769）が適用されていない場合、  
>　<font color="Red">オフにすると二度と戻せません</font>ので注意してください。  
>\
>バージョン 24H2,25H2 に2026年4月累積更新（KB5083769）以降が  
>適用されていると下記の設定変更が追加されています。  
>\
>スタートメニューを右クリック → 設定 →  
>プライバシーとセキュリティ → Windows セキュリティ →  
>アプリとブラウザの制御 → 評価ベースの保護設定 →  
>「望ましくない可能性のあるアプリのブロック」を解除。
...
>確認方法は[こちら](ft_webcamera.html)を参照してください。  
...
>\
><font color="Red">ウェブカメラの同時使用については[こちら](DeviceWebCamera.html)を参照してください。</font>
...
>\
>接続方法については[こちら](ft_iphone.html)を参照してください。
...
>LeapMotion が動かない場合は[ソフトウェアのバージョン](leapmotion.html)を確認してください。  
>\
...
>[こちら](Nuitrack.html) で手順を確認してください。
...
>\
>なお、OBS との連携では[仮想ウェブカメラ](VirtualWebCamera.html)を経由した  
...
>Mac 版は非対応の為、表示されません。  
>\
>Steam 版で利用するには[手動登録](VirtualWebCamera.html)が必要となります。  
>\
>3tene をインストール後にフォルダ移動した場合も動作しなくなりますので  
>移動後のフォルダで仮想カメラの[手動登録](VirtualWebCamera.html)を行ってください。  
>\
>FREE と PRO を両方インストールした後に片方をアンインストールした場合も  
>動作しなくなりますので仮想カメラの[手動登録](VirtualWebCamera.html)を行ってください。
...
>有効になっているのを確認してください。  
>\
>有効になっている場合は仮想カメラの[手動登録](VirtualWebCamera.html)を行ってください。
...
>\
>ショートカットの[注意事項](settingShortcut.html)
